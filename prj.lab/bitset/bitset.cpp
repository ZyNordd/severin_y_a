...
Bitset::Bitset(const int64_t size, const bool filler) {
    size_ = size;
    clusters_count_ = (size - 1) / kClusterBitSize + 1;
    if (size_ < 0) {
    ...
    data_ = new uint8_t[clusters_count_];
...
Bitset::Bitset(const std::string& rhs) : Bitset(rhs.size()) {
    for (int64_t i = 0; i < size_; ++i) {
        if (rhs[size_ - i - 1] != '0' &&
            rhs[size_ - i - 1] != '1') {
            throw std::invalid_argument("Bitset error: Bitset must consist of 0 and 1 only");
        }
        data_[i / kClusterBitSize] |= (rhs[size_ - i - 1] - '0') <<
            (i % kClusterBitSize);
    }
    return;
}

Bitset::Bitset(const Bitset& copy) : Bitset(copy.size()) {
    for (int64_t i = 0; i < clusters_count_; ++i) {
...
Bitset::BitHolder::BitHolder(Bitset* bitset, const int64_t position) {
    ...
    position_ = position;
    if (position_ < 0) {
        throw std::invalid_argument("BitHolder error: negative position is undefined");
    }
    return;
}
...
Bitset::BitHolder& Bitset::BitHolder::operator=(const bool rhs) {
    uint8_t& cluster_ptr = (bitset_->data_)[position_ / kClusterBitSize];
    if (rhs) {
...
Bitset& Bitset::operator<<=(const int64_t rhs) {
    ...
    int64_t bit_delta = rhs % kClusterBitSize;
    {
        ...
        for (; i > clusters_delta; --i) {
            data_[i] = static_cast<uint8_t>(data_[i - clusters_delta] <<
                bit_delta);
...
Bitset& Bitset::operator>>=(const int64_t rhs) {
    ...
    int64_t bit_delta = rhs % kClusterBitSize;
    {
        ...
        for (; i < clusters_count_ - clusters_delta - 1; ++i) {
            data_[i] = data_[i + clusters_delta] >> bit_delta;
            data_[i] |= static_cast<uint8_t>(data_[i + clusters_delta + 1] <<
                (kClusterBitSize - bit_delta));
        }
        data_[i] = data_[i + clusters_delta] >> bit_delta;
        ++i;
...
Bitset Bitset::operator~() const {
    ...
    if (size_ != clusters_count_ * kClusterBitSize) {
        lhs.data_[clusters_count_ - 1] &= static_cast<uint8_t>(~0) >>
...
Bitset::BitHolder Bitset::operator[](const int64_t position) {
    ...
    return Bitset::BitHolder(this, position_);
}
